...
function addPayment() {
    ...
    var paymentsID = document.getElementsByClassName("paymentsID")
    var PaymentType = document.getElementsByClassName("PaymentType")
   
    var ChequeType = document.getElementsByClassName("ChequeType")
    var txtChaqueNo = document.getElementsByClassName("txtChaqueNo")
    var ChaqueDateC = document.getElementsByClassName("ChaqueDate")
    var txtPostedDateC = document.getElementsByClassName("txtPostedDate")
    var txtClearingDateC = document.getElementsByClassName("txtClearingDate")
    var txtTransection = document.getElementsByClassName("txtTransection")
    var TransectionDateC = document.getElementsByClassName("TransectionDate")
    var txtDate = document.getElementsByClassName("txtDate")
    var txtNarration = document.getElementsByClassName("txtNarration")
    var Highlighter = document.getElementsByClassName("divColor")

    var ChaqueDate, TransectionDate, txtClearingDate, txtPostedDate
    for (var i = 0; i < paymentsID.length; i++) {

      

        if (ChaqueDateC[i].value == '') {
            ChaqueDate = '1900-01-01'
        }
        else {
            ChaqueDate = ChaqueDateC[i].value
        }

        if (TransectionDateC[i].value == '') {
            TransectionDate = '1900-01-01'
        }
        else {
            TransectionDate = TransectionDateC[i].value
        }

        if (txtClearingDateC[i].value == '') {
            txtClearingDate = '1900-01-01'
        }
        else {
            txtClearingDate = txtClearingDateC[i].value
        }

        if (txtPostedDateC[i].value == '') {
            txtPostedDate = '1900-01-01'
        }
        else {
            txtPostedDate = txtPostedDateC[i].value
        }



        var ttfor = $("#" + paymentsID[i].id +" option:selected").text();
        var ttfor1 = ttfor.split("[");

        var narS = txtNarration[i].value + ' [From: <a id=' + srcValue + ' href="#" onclick=loadpage(this)><u>' + srcText + '</u><a>]'; //"Paid for Testing';
        var narT = txtNarration[i].value + ' [To: <a id=' + paymentsID[i].value + ' href="#" onclick=loadpage(this)><u>' + ttfor1[0] + '</u><a>]'; //"Paid for Testing';

        if (typeof paymentsID[i].value == 'undefined' || typeof srcValue == 'undefined' || txtAmount[i].value == "") {
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: "{'Source':'" + srcValue + "','TrDate':'" + txtDate[i].value + "','TrAccount':'" + paymentsID[i].value + "','Amount':'" + txtAmount[i].value + "','NarrationS':'" + narS + "','NarrationT':'" + narT + "','Type':'" + type + "','TType':'" + TType + "','LID':'" + LID1 + "','UserID':'" + userid + "','FileID':'" + FileID + "','Highlighter':'" + Highlighter[i].style.backgroundColor + "','TransectionId':'" + txtTransection[i].value + "','TransectionDate':'" + TransectionDate + "','ChaqueNo':'" + txtChaqueNo[i].value + "','ChaqueDate':'" + ChaqueDate + "','ChaquePostedDate':'" + txtPostedDate + "','ChaqueClaringDate':'" + txtClearingDate + "','PaymentType':'" + PaymentType[i].value + "','ChequeType':'" + ChequeType[i].value + "'}",
                contentType: 'application/json; charset=utf-8',
                success: function (response) {
                    //alert(response.d);
                    if (TType == "New") {
